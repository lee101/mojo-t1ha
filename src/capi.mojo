...
def load64(data: U8Ptr, start: Int) -> UInt64:
    ...
    if (address & 7) == 0:
        return U64Ptr(unsafe_from_address=address).load(0)
    var value = UInt64(0)
    for j in range(8):
        value |= UInt64(data.load(start + j)) << UInt64(j * 8)
    return value
...
def update_block(state: U64Ptr, data: U8Ptr, start: Int):
    ...
    state.store(3, d ^ (b + rot64(w1, UInt64(38))))
    state.store(2, c ^ (a + rot64(w0, UInt64(57))))
    state.store(1, b ^ (P6 * (c13 + w2)))
    state.store(0, a ^ (P5 * (d02 + w3)))
...
def squash(state: U64Ptr):
    ...
    var c = state.load(2)
    var d = state.load(3)
    state.store(0, a ^ (P6 * (c + rot64(d, UInt64(23)))))
    state.store(1, b ^ (P5 * (rot64(c, UInt64(19)) + d)))


def mix_ab(state: U64Ptr, left: Int, right: Int, value: UInt64, prime: UInt64):
    var product = state.load(right) + value
    state.store(left, state.load(left) ^ (product * prime))
    state.store(right, state.load(right) + mul_high(product, prime))


def final128(state: U64Ptr, result: U64Ptr):
    mix_ab(state, 0, 1, rot64(state.load(2), UInt64(41)) ^ state.load(3), P0)
    mix_ab(state, 1, 2, rot64(state.load(3), UInt64(23)) ^ state.load(0), P6)
    mix_ab(state, 2, 3, rot64(state.load(0), UInt64(19)) ^ state.load(1), P5)
    mix_ab(state, 3, 0, rot64(state.load(1), UInt64(31)) ^ state.load(2), P4)
    result.store(0, state.load(2) + state.load(3))
    result.store(1, state.load(0) ^ state.load(1))
...
def tail_ab(state: U64Ptr, data: U8Ptr, start: Int, length: Int) -> UInt64:
    ...
    return final64(state.load(0), state.load(1))
...
def init_state(state: U64Ptr, x: UInt64, y: UInt64):
    state.store(0, x)
    state.store(1, y)
    state.store(2, rot64(y, UInt64(23)) + ~x)
    state.store(3, ~y + rot64(x, UInt64(19)))


def atonce64(data: U8Ptr, length: Int, seed: UInt64) -> UInt64:
    var state_storage = InlineArray[UInt64, 4](fill=0)
    var state = U64Ptr(to=state_storage[0])
    state.store(0, seed)
    state.store(1, UInt64(length))
    var pos = 0
    if length > 32:
        state.store(2, rot64(UInt64(length), UInt64(23)) + ~seed)
        state.store(3, ~UInt64(length) + rot64(seed, UInt64(19)))
        while pos + 32 <= length:
...
def mt1_stream_init(state_addr: Int, meta_addr: Int, seed_x: UInt64, seed_y: UInt64) abi("C"):
    ...
    meta.store(0, UInt64(0))
    meta.store(1, UInt64(0))
...
def mt1_stream_update(state_addr: Int, buffer_addr: Int, meta_addr: Int, data_addr: Int, length: Int) abi("C"):
    ...
    var partial = Int(meta.load(0))
    meta.store(1, meta.load(1) + UInt64(length))
    var pos = 0
    if partial > 0:
        var needed = 32 - partial
        var chunk = needed if length >= needed else length
        for i in range(chunk):
            buffer.store(partial + i, data.load(i))
        partial += chunk
        pos += chunk
        if partial < 32:
            meta.store(0, UInt64(partial))
            return
    ...
    for i in range(remaining):
        buffer.store(i, data.load(pos + i))
    meta.store(0, UInt64(remaining))
...
def mt1_stream_final(state_addr: Int, buffer_addr: Int, meta_addr: Int, result_addr: Int) abi("C"):
    ...
    var buffer = U8Ptr(unsafe_from_address=buffer_addr)
    var meta = U64Ptr(unsafe_from_address=meta_addr)
    var total_bits = (meta.load(1) << UInt64(3)) ^ (UInt64(1) << UInt64(63))
    var bits = InlineArray[UInt64, 1](fill=total_bits)
    mt1_stream_update(state_addr, buffer_addr, meta_addr, Int(U64Ptr(to=bits[0])), 8)
    tail_abcd(state, buffer, 0, Int(meta.load(0)), U64Ptr(unsafe_from_address=result_addr))
